# Task packet T-deploy - Deploy and observe

Run: `20260910T114536Z-minifleet-v02-40d5be`   Role: **builder**   Branch: `task/deploy-a2`

## Goal

Implement minifleet/deploy.py: supervised process launch with ready detection, plus smoke and soak probes.
...
- **A-3** (operability): A supervisor starts a process, parses its ready line and port, times out without hanging when no ready line arrives, and always tears the process down
- **A-4** (operability): Smoke and soak probes report status, sample count, failure count and latency percentiles
...
```json
{"task_id": "T-deploy", "status": "submitted|failed", "files": ["..."], "commands": ["..."], "notes": "what you did and what you did not do"}
```
